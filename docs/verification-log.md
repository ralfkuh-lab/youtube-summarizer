...
- 2026-09-20 (Nachtrag 2): Zweite Rückmeldung des Maintainers: kurzes Datumsformat mit Uhrzeit
  ...
  `available.chats[].firstQuestion`, auf 1000 Zeichen begrenzt; H7 erweitert), neuer Fall G35; U35 auf
  das neue Datumsformat umgestellt. `cargo fmt --check`, `cargo test` (299 bestanden, 4 ignoriert),
  `npm run build`, `npm run test:ui` (104 Fälle; im Gesamtlauf U35 rot wegen des alten Datumsformats,
  nach der Umstellung gezielt grün, Gesamtlauf danach nicht wiederholt), `npm run tauri -- build` grün;
  Screenshot geprüft.

- 2026-09-20 (Nachtrag): Rückmeldung des Maintainers aus der installierten App: Haken und Beschriftung
  standen untereinander (`.modal-content label` erzwingt Spaltenlayout und schlug `.chat-context-choice`),
  „Kontext“ stand gleichrangig neben seinen Untergruppen, Radios überflüssig. Vom Orchestrator umgebaut:
  eigene Klassen `agent-context-*`, „Kontext“ als Feldbeschriftung, Radios entfernt (`null` erscheint als
  angehakte neueste Version), Shell-Hinweis an „Kommando“. Tests G12/G14/G28–G30/G32 umgestellt, neu G33
  (Altbestand) und G34 (Haken und Text auf einer Höhe, per `boundingBox`). `npm run build`,
  `npm run test:ui` (103 bestanden), `npm run tauri -- build` grün; Screenshot des Dialogs geprüft
  (`AGENT_DIALOG_SCREENSHOT=<pfad>` im Fall G34). Lehre: Mock-UI-Tests belegen kein Layout — bei neuen
  Dialogen vor der Abnahme einen Screenshot ansehen.

- 2026-09-20: Kontextauswahl im Übergabe-Dialog (Spec Revision 3 in `docs/spec-agent-handoff.md`) und
  Eigennamen-Hinweis. Implementierung DeepSeek, Reviews Gemini und GLM 5.3 Flash (paralleler Vergleich),
  Nachprüfung Gemini. Korrekturpaket K1–K5: Auswahlzustand synchron geführt (Geminis Befund: schnelle
  Klickfolgen überschrieben sich; G28–G30), Mock-Vorbelegung aus der Konfiguration (G31), Leeres nicht
  wählbar (H13–H15), „1 Nachricht“, Drahtform-Test, Tastaturfokus (G32). Gates vom Orchestrator
  nachgefahren: `cargo fmt --check`, `cargo test` (299 bestanden, 4 ignoriert), `npm run build`,
  `npm run test:ui` (101 bestanden; ein früherer Lauf parallel zu Änderungen des Implementierers hatte
  zwei Timeouts G18/U18, am Endstand nicht reproduziert), `npm run tauri -- build` (deb 11:39).
  Mutationsnachweise H3, H4, H8, H13, G13, G19, G28, G29, G32 in `/tmp/yts-mut-ctx` laut
  `.herd/impl-ctx-bericht.md` (vom Orchestrator nicht einzeln nachgefahren). Nativer Durchlauf mit
  `npm run tauri dev` (isolierte Datenkopie, danach gelöscht) über `POST /api/agent-handoff/161`:
  Vorbelegung `{true,null,[]}` (17 727 Zeichen), Auswahl `[999999,1]` → `[]` mit Abwahl-Hinweis und
  ohne Eigennamen-Hinweis (1 062 Zeichen), Auswahl `[155,154]`+Chat 3 → zwei SUMMARY-Blöcke älteste
  zuerst mit eigenen Delimitern und ein CHAT-Block. Nicht nativ geprüft: der Dialog unter WebKitGTK.
  Dev-App beendet.
